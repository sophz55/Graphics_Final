...
void generate_sphere( struct matrix * points, 
  ...
  for (i = step; i < 1 + step; i += step){
    for (j = step; j < 1 + step; j += step){
    x = cx + r * cos( 2 * M_PI * j );
    y = cy + r * sin( 2 * M_PI * j ) * cos( M_PI * i );
    z = r * sin( 2 * M_PI * j ) * sin(M_PI * i );
...
void generate_torus( struct matrix * points, 
  ...
  for (i = step; i < 1 + step; i += step){
    for (j = step; j < 1 + step; j += step){
    x = cx + r1 * cos( 2 * M_PI * j );
    y = cy + ( r1 * sin( 2 * M_PI * j ) + r2 ) * cos( 2 * M_PI * i );
    z = ( r1 * sin( 2 * M_PI * j ) + r2 ) * sin( 2 * M_PI * i );
    add_point( points, x, y, z );
    }
  }
}
